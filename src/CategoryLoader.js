...
export function loadCategories(iconIds) {
    ...
    for (const catId in categories) {
      for (const part of partsByIconId[iconId]) {
        if (part.match(categories[catId].regex)) {
          if (!categoryInfoByIconId[iconId])
            categoryInfoByIconId[iconId] = {
              rootCategories: [],
              allCategories: [],
            };
          if (!categoryInfoByIconId[iconId].rootCategories.includes(catId))
            categoryInfoByIconId[iconId].rootCategories.push(catId);
          ...
          while (catsToPopulate.length) {
            ...
            if (categories[catId].super) {
              catsToPopulate = catsToPopulate.concat(
                stringArray(categories[catId].super),
              );
            }
            if (!categoryInfoByIconId[iconId].allCategories.includes(catId))
              categoryInfoByIconId[iconId].allCategories.push(catId);
          }
          break;
        }
      }
  ...
  for (const cat in categories) {
    ...
    if (categories[cat].icons?.length === 1) {
      ...
      for (const iconId in categoryInfoByIconId) {
        const rootIndex =
          categoryInfoByIconId[iconId].rootCategories.indexOf(cat);
        if (rootIndex >= 0)
          categoryInfoByIconId[iconId].rootCategories.splice(rootIndex, 1);

        const allIndex =
          categoryInfoByIconId[iconId].allCategories.indexOf(cat);
        if (allIndex >= 0)
          categoryInfoByIconId[iconId].allCategories.splice(allIndex, 1);
      }
    }
  }
  ...
  for (const iconId in categoryInfoByIconId) {
    categoryInfoByIconId[iconId].commons = categoryInfoByIconId[
      iconId
    ].rootCategories
      .map((rootCatId) => {
