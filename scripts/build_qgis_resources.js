...
function copySvgs(sourceDir, destDir, majorVersion) {
  const changelog = changelogs.find(
    (log) => parseInt(log.majorVersion) === majorVersion,
  );
  if (!changelog)
    throw new Error(`cannot find changelog for version ${majorVersion}`);
  const iconIds = new Set(
    changelog.iconChanges.map((change) => change.newId).filter(Boolean),
  );
