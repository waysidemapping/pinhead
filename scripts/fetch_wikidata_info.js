...
const wikidataData = await fetchWikidataEntities(allUniqueEntities, [
  "P13786",
  "P1282",
]);
      ...
      if (entity.statements && entity.statements[prop]) {
        for (const statement of entity.statements[prop]) {
          ...
          if (qid) {
            ...
            if (!dataByPinheadId[pinheadId][attr])
              dataByPinheadId[pinheadId][attr] = [];
            if (!dataByPinheadId[pinheadId][attr].includes(qid))
              dataByPinheadId[pinheadId][attr].push(qid);

            const info = wikidataData[qid];
            if (info) {
              const osmTags = (info.statements?.P13786 || []).concat(
                info.statements?.P1282 || [],
              );
              if (osmTags.length) {
                if (!dataByPinheadId[pinheadId].osmTags)
                  dataByPinheadId[pinheadId].osmTags = [];
                dataByPinheadId[pinheadId].osmTags =
                  dataByPinheadId[pinheadId].osmTags.concat(osmTags);
              }

              if (info.labels) {
                for (const lang in info.labels) {
                  if (!translationsByLang[lang])
                    translationsByLang[lang] = { icons: {} };
                  if (!translationsByLang[lang].icons[pinheadId])
                    translationsByLang[lang].icons[pinheadId] = {};
                  if (!translationsByLang[lang].icons[pinheadId].labels)
                    translationsByLang[lang].icons[pinheadId].labels = [];
                  const labelLC = info.labels[lang].toLowerCase();
                  if (
                    !translationsByLang[lang].icons[pinheadId].labels.includes(
                      labelLC,
                    )
                  ) {
                    translationsByLang[lang].icons[pinheadId].labels.push(
                      labelLC,
                    );
                  }
                }
              ...
              if (info.aliases) {
                for (const lang in info.aliases) {
                  ...
                  if (!translationsByLang[lang].icons[pinheadId])
                    translationsByLang[lang].icons[pinheadId] = {};
                  if (!translationsByLang[lang].icons[pinheadId].aliases)
                    translationsByLang[lang].icons[pinheadId].aliases = [];
                  for (const alias of info.aliases[lang]) {
                    ...
                    if (
                      ...
                      !translationsByLang[lang].icons[
                        pinheadId
                      ].aliases.includes(aliasLC)
                    ) {
                      translationsByLang[lang].icons[pinheadId].aliases.push(
                        aliasLC,
                      );
                    }
                  }
...
writeFileSync(
  "docs/commons_metadata.json",
  JSON.stringify({
    date: new Date().toISOString(),
    iconStatements: dataByPinheadId,
  }),
);
for (const lang in translationsByLang) {
  writeFileSync(
    `docs/translations/${lang}.json`,
    JSON.stringify(translationsByLang[lang]),
  );
}
