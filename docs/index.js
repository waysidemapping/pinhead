...
async function setupPage(pageData) {
  ...
  const newIconIds = currentChangelog.iconChanges
    .filter(iconChange => iconChange.newId && (!iconChange.oldId || ((iconChange.by || iconChange.src) && !["flip", "resize", "minor", "merge"].includes(iconChange.edit))))
    ...
    .insertAdjacentHTML("afterbegin", [
      ...
      new Chainable('a')
        ...
        .append(
          new Chainable('img')
            .setAttribute('class', 'inline-icon')
            .setAttribute('src', `/latest/arrow_top_right_from_square_outline.svg`),
          new Chainable('span')
            .append('github')
        )),
           new Chainable('li')
      ...
      new Chainable('a')
        ...
        .append(
          new Chainable('img')
            .setAttribute('class', 'inline-icon')
            .setAttribute('src', `/latest/arrow_right_from_left_bracket.svg`),
          new Chainable('span')
            .append('changelog')
        )),
         new Chainable('li')
  ...
  function reloadInspector(iconId) {
    ...
    new Chainable('div')
      .setAttribute('id', 'inspector-body')
      .append(
        new Chainable('div')
        .setAttribute('id', 'inspector-top')
        .append(
              new Chainable('div')
    ...
    inspector.querySelectorAll('a.close')
    ...
    inspector.querySelectorAll('textarea')
      .forEach(el => inspector.addEventListener('focus', e => e.target.select()));
...
function filterIcons(rawQuery) {
  ...
  for (const element of elements) {
    ...
    if (query === '' || iconId.replaceAll('_', '').includes(query) || matchesALabel) {
